          ...
          <div className="divide-y divide-line">
            {flagged.map((p) => (
              <div key={p.id} className="card-body flex items-center justify-between gap-4">
                ...
                <Link href={`/registry/property/${p.id}`} className="btn-secondary text-sm shrink-0">Review</Link>
              </div>
            ))}
          </div>
        </div>
                ...
                {results.map((r) => {
                  ...
                  return (
                    <tr key={r.id}>
                      ...
                      <td><Link href={`/registry/resident/${r.id}`} className="text-brass font-medium hover:underline">View</Link></td>
                    </tr>
                  );
                })}
              </tbody>
