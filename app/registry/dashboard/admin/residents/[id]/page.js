...
export default async function ResidentProfilePage({ params, searchParams }) {
  ...
  if (!resident) {
    return (
      <Shell user={profile}>
        <Link href="/registry/dashboard/admin/residents" className="inline-flex items-center gap-1.5 text-sm font-medium text-muted hover:text-ink">
  ...
  return <ResidentProfileClient user={profile} resident={resident} access={access} />;
}