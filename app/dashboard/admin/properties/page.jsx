...
import { WARDS, formatDate, statusLabel } from "@/lib/data";
...
/** Property-centric view of the registry — complements the resident-centric
 *  table on Overview. One row per property: who owns it, how many units,
 *  how many are occupied, and its verification status. */
export default function PropertiesPage() {
  ...
  const results = db.properties.filter((p) => {
    ...
    if (query.trim()) {
      ...
      const haystack = [p.property_number, p.address, owner?.name].filter(Boolean).join(" ").toLowerCase();
    ...
    <Shell user={user}>
      ...
      <div className="card">
        <div className="card-header flex-wrap gap-3">
          <h2 className="font-display text-base text-ink font-semibold">{results.length} of {db.properties.length}</h2>
          <div className="flex flex-wrap gap-2">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Address, owner, property ID"
                value={query}
        ...
        {results.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="ledger-table">
              <thead>
                <tr><th>Property</th><th>Owner</th><th>Ward</th><th>Units</th><th>Occupied</th><th>Status</th><th></th></tr>
              </thead>
              <tbody>
                {results.map((p) => {
                  const owner = db.owners.find((o) => o.id === p.owner_id);
                  const w = WARDS.find((x) => x.id === p.ward_id);
                  ...
                  return (
                    <tr key={p.id}>
                      <td>
                        ...
                        <p className="text-xs text-muted">Added {formatDate(p.created_at)}</p>
                      </td>
                      ...
                      <td><Stamp id={statusLabel(p.status)} status={p.status} hideDot /></td>
                      <td><Link href={`/property/${p.id}`} className="text-brass font-medium hover:underline">View</Link></td>
                    </tr>
                  );
                })}
              </tbody>
