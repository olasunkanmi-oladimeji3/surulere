...
export default function ResidentProfilePage() {
  ...
  const property = db.properties.find((p) => p.id === resident.property_id);
  const unit = property?.units.find((u) => u.id === resident.unit_id);

  const isSelf = user.role === "resident" && user.id === resident.id;
  const isAdmin = user.role === "admin";
  const isOwner = user.role === "owner" && property?.owner_id === user.id;
  const isAssignedCda = user.role === "cda" && property && user.ward_id === property.ward_id;
  const hasAccess = isSelf || isAdmin || isOwner || isAssignedCda;
  ...
  const canSeeFullNin = isSelf || isAdmin || isOwner;
    ...
    <Shell user={user}>
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 mt-4 mb-6">
        <div>
          <div className="flex items-center gap-2.5 flex-wrap">
            ...
            {property && <span className="text-xs text-muted">Property {property.property_number}</span>}
          </div>
      ...
      {property && unit && (
        <Section title="Property & unit">
          <Field label="Property number" value={property.property_number} mono />
      ...
      {resident.household_members?.length > 0 && (
        <Section title={`Other household members — ${resident.household_members.length}`}>
          <div className="sm:col-span-3">
            <ul className="divide-y divide-line">
              {resident.household_members.map((m) => (
      ...
      {property?.staff?.length > 0 && (
        <Section title={`Domestic staff at this property — ${property.staff.length}`}>
          <div className="sm:col-span-3">
            <ul className="divide-y divide-line">
      ...
      {property && (
        <div className="card mt-6">
          ...
          <div className="card-body">
            {db.verificationLogs.filter((l) => l.property_id === property.id).length === 0 ? (
              <p className="text-sm text-muted">No visits logged yet.</p>
            ) : (
              <ul className="space-y-3">
                {db.verificationLogs.filter((l) => l.property_id === property.id).slice().reverse().map((log) => {
...
function Section({ title, children }) {
  return (
    <div className="card mb-6">
      <div className="card-header"><h2 className="font-display text-base text-ink font-semibold">{title}</h2></div>
