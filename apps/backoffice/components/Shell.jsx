...
/** This app is admin-only — one nav, flat paths (see app/login/page.js for
 *  the role check that keeps other roles out). */
const NAV_ITEMS = [
  { href: "/", label: "Overview", icon: "building" },
  { href: "/properties", label: "Properties", icon: "house" },
  { href: "/residents", label: "Residents", icon: "doorOpen" },
  { href: "/field-submissions", label: "Field submissions", icon: "flag" },
  { href: "/cda-members", label: "CDA members", icon: "shield" },
  { href: "/lg-staff", label: "LG Staff", icon: "users" },
  { href: "/audit-log", label: "Audit log", icon: "search" },
];

// The citizen/self-service registry lives in a separate app/deployment —
// this backoffice only has the LG Staff dashboard. Set this to that app's
// real URL once deployed; falls back to a relative link in local dev, where
// registry usually runs on :3000 alongside this app on :3001.
      ...
      <aside className="relative bg-ink text-on-ink lg:w-64 lg:min-h-screen lg:fixed lg:inset-y-0 flex flex-col overflow-hidden">
        ...
        <div className="relative flex items-center gap-2.5 px-5 py-5 border-b border-white/10">
          ...
          <div className="leading-tight">
            ...
            <p className="text-[11px] text-on-ink/60">Staff Backoffice</p>
          </div>
        </div>
        <nav className="flex-1 px-3 py-4 space-y-1">
          ...
          <a href={REGISTRY_URL} className="side-link">
            <Icon name="house" /> Registry site ↗
          </a>
        </nav>
