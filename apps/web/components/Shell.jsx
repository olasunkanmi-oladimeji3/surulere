...
// The LG Staff dashboard is a separate app now (see apps/backoffice) — an
// admin viewing a shared page here (e.g. a property/resident detail page,
// which they do have access to) gets sent there for everything else.
...
const NAV_ITEMS = {
  ...
  admin: [
    { href: BACKOFFICE_URL, label: "Staff backoffice ↗", icon: "building", external: true },
  ],
};
    ...
    <div className="min-h-screen flex flex-col lg:flex-row">
      <aside className="relative bg-ink text-on-ink lg:w-64 lg:min-h-screen lg:fixed lg:inset-y-0 flex flex-col overflow-hidden">
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          {navItems.map((item) =>
            item.external ? (
              <a key={item.href} href={item.href} target="_blank" rel="noopener noreferrer" className="side-link">
        ...
        {navItems.length > 1 && (
          <nav className="flex gap-1 px-5 pt-3 lg:hidden overflow-x-auto">
            {navItems.map((item) =>
              item.external ? (
                <a
                  key={item.href} href={item.href} target="_blank" rel="noopener noreferrer"
                  className="text-xs font-medium px-3 py-1.5 rounded-[var(--radius-pill)] shrink-0 bg-paper text-muted"
                >
                  {item.label}
                </a>
              ) : (
