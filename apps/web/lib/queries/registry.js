...
/** Same three counts as getDashboardCounts, scoped to one ward — for a CDA member's own-ward overview. */
export async function getPropertyStatusCountsForWard(supabase, wardId) {
  const [{ count: verified }, { count: pending }, { count: flagged }] = await Promise.all([
    supabase.from("properties").select("*", { count: "exact", head: true }).eq("ward_id", wardId).eq("status", "verified"),
    supabase.from("properties").select("*", { count: "exact", head: true }).eq("ward_id", wardId).eq("status", "pending"),
    supabase.from("properties").select("*", { count: "exact", head: true }).eq("ward_id", wardId).eq("status", "flagged"),
  ]);
  return { verified, pending, flagged };
}

/** verification_logs entries a specific actor (e.g. a CDA member) has personally logged, newest first. */
export async function getVerificationLogsByActor(supabase, actorId, limit = 5) {
  const { data, error } = await supabase
    .from("verification_logs")
    .select("id, outcome, note, created_at, property_id, properties ( property_number )")
    .eq("actor_id", actorId)
