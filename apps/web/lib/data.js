...
// LG Staff (admin) have no dashboard in this app anymore — see
// apps/backoffice, a separate deployment. Sending an admin there directly
// on login (rather than into this app first) avoids a dead-end bounce.
const BACKOFFICE_URL = process.env.NEXT_PUBLIC_BACKOFFICE_URL || "http://localhost:3001";

export function navHrefFor(role) {
  return {
    owner: "/registry/dashboard/owner",
    resident: "/registry/dashboard/resident",
    cda: "/registry/dashboard/cda",
    admin: BACKOFFICE_URL,
  }[role] || "/registry/login";
}
...
/* Real auth (login, password changes, sessions) runs entirely through
 * Supabase Auth — see contexts/AuthContext.jsx and lib/supabaseData.js.
 * Supabase hashes passwords server-side (bcrypt); this app never sees or
 * stores a plaintext password. The mock Auth object that used to live here
 * (plaintext password compare against localStorage) has been removed — it
 * was already fully dead code, unreferenced anywhere in the app. */
